        ...
        <div className="mt-4 space-y-3">
          {cadastros.map((cadastro) => {
            ...
            return <article key={cadastro.userId} className="rounded-lg border border-border">
              <Button variant="ghost" onClick={() => setAberto(aberto === cadastro.userId ? null : cadastro.userId)} className="h-auto w-full justify-start gap-3 p-4 text-left">
              ...
              {aberto === cadastro.userId && <div className="border-t border-border p-4">
                <div className="grid gap-4 lg:grid-cols-2">
                  <div><h3 className="text-sm font-bold">Documentos enviados</h3><div className="mt-2 space-y-2">{cadastro.documentos.map((doc) => <Button key={doc.id} variant="secondary" onClick={() => void abrirDocumento(doc.id)} className="h-auto w-full justify-between p-3 text-left text-xs"><span>{doc.tipo.replaceAll("_", " ")} · {doc.status}</span><ExternalLink className="size-3.5" /></Button>)}{cadastro.documentos.length === 0 && <p className="text-xs text-muted-foreground">Nenhum documento enviado.</p>}</div></div>
